...
export async function nextSearch(circleCoordinates, searchID, checksum, searchType, testMode) {
  ...
  try {
    let response = await getNextSearchCoord(circleCoordinates, searchID, checksum)
      ...
      if (testMode) {
        ...
        let radius_and_googleData = await dummyGoogleCall()
      ...
      try {
        console.log('running paid search')
